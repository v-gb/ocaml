...
val fprintf : out_channel -> format('a, out_channel, unit) -> 'a
...
val printf : format('a, out_channel, unit) -> 'a
(** Same as {!Printf.fprintf}, but output on [stdout]. *)

val eprintf : format('a, out_channel, unit) -> 'a
...
val bprintf : Buffer.t -> format('a, Buffer.t, unit) -> 'a
(** Same as {!Printf.fprintf}, but instead of printing on an output channel,
   append the formatted arguments to the given extensible buffer
   (see module {!Buffer}). *)

val ifprintf : 'b -> format4('a, 'b, 'c, unit) -> 'a
(** Same as {!Printf.fprintf}, but does not print anything.
    Useful to ignore some material when conditionally printing.
    @since 3.10
*)

val ibprintf : Buffer.t -> format('a, Buffer.t, unit) -> 'a
...
val kprintf : (string -> 'b) -> format4('a, unit, string, 'b) -> 'a
