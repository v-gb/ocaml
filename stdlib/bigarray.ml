...
let kind_size_in_bytes : type a b. kind(a, b) -> int = function
  | Float16 -> 2
  | Float32 -> 4
  | Float64 -> 8
  | Int8_signed -> 1
...
module Genarray = struct
  ...
  external create: kind('a, 'b) -> layout('c) -> array(int) -> t('a, 'b, 'c)
     = "caml_ba_create"
  external get: t('a, 'b, 'c) -> array(int) -> 'a
     = "caml_ba_get_generic"
  ...
  let init (type t) kind (layout : layout(t)) dims f =
  ...
  external sub_left: t('a, 'b, c_layout) -> int -> int -> t('a, 'b, c_layout)
     = "caml_ba_sub"
  external sub_right: t('a, 'b, fortran_layout) -> int -> int ->
                          t('a, 'b, fortran_layout)
     = "caml_ba_sub"
  external slice_left: t('a, 'b, c_layout) -> array(int) ->
                          t('a, 'b, c_layout)
     = "caml_ba_slice"
  external slice_right: t('a, 'b, fortran_layout) -> array(int) ->
                          t('a, 'b, fortran_layout)
     = "caml_ba_slice"
...
module Array1 = struct
  ...
  external sub: t('a, 'b, 'c) -> int -> int -> t('a, 'b, 'c) = "caml_ba_sub"
  let slice (type t) (a : Genarray.t(_, _, t)) n =
    match layout a with
    | C_layout -> (Genarray.slice_left a [|n|] : Genarray.t(_, _, t))
    | Fortran_layout -> (Genarray.slice_right a [|n|]: Genarray.t(_, _, t))
  external blit: t('a, 'b, 'c) -> t('a, 'b, 'c) -> unit = "caml_ba_blit"
  external fill: t('a, 'b, 'c) -> 'a -> unit = "caml_ba_fill"
...
module Array2 = struct
  ...
  let init (type t) kind (layout : layout(t)) dim1 dim2 f =
...
module Array3 = struct
  ...
  let init (type t) kind (layout : layout(t)) dim1 dim2 dim3 f =
...
external genarray_of_array0: Array0.t('a, 'b, 'c) -> Genarray.t('a, 'b, 'c)
   = "%identity"
external genarray_of_array1: Array1.t('a, 'b, 'c) -> Genarray.t('a, 'b, 'c)
   = "%identity"
external genarray_of_array2: Array2.t('a, 'b, 'c) -> Genarray.t('a, 'b, 'c)
   = "%identity"
external genarray_of_array3: Array3.t('a, 'b, 'c) -> Genarray.t('a, 'b, 'c)
   = "%identity"
...
external reshape:
   Genarray.t('a, 'b, 'c) -> array(int) -> Genarray.t('a, 'b, 'c)
   = "caml_ba_reshape"
